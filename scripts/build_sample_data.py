...
def main() -> None:
    ...
    widget = {f: (None if pd.isna(sample.iloc[0][f]) else _clean(sample.iloc[0][f])) for f in FEATURES}
    Path("data/widget/sample_patient.json").write_text(json.dumps(widget, indent=2), encoding="utf-8")
